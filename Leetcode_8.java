...
public class Leetcode_8 {
    ...
    private int u2172;
    private int slots2172;

    public int maximumANDSum(int[] nums, int numSlots) {
        int n = nums.length;
        // 看成有 numSlots * 2 个篮子，每个篮子可放最多1个数
        this.slots2172 = numSlots << 1;
        this.u2172 = (1 << n) - 1;
        this.nums2172 = nums;
        this.memo2172 = new int[slots2172][1 << n];
        for (int i = 0; i < slots2172; ++i) {
    ...
    private int dfs2172(int i, int j) {
        if (j == u2172) {
            return 0;
        }
        if (slots2172 - i < Integer.bitCount(u2172 ^ j)) {
            return Integer.MIN_VALUE / 2;
        }
        if (memo2172[i][j] != Integer.MIN_VALUE) {
            return memo2172[i][j];
        }
        int res = dfs2172(i + 1, j);
        for (int c = u2172 ^ j; c != 0; c &= c - 1) {
            int lb = Integer.numberOfTrailingZeros(c);
            res = Math.max(res, dfs2172(i + 1, j | (1 << lb)) + (((i >> 1) + 1) & nums2172[lb]));
        }
        return memo2172[i][j] = res;
    }
    ...
    private int dfs3003(int i, int j, int changed) {
        if (i == s3003.length()) {
            return 1;
        }
        long key = (long) i << 27 | (changed << 26) | j;
        if (memo3003.containsKey(key)) {
            return memo3003.get(key);
        }
        int res = 0;
        // 不变
        int bits = j | (1 << (s3003.charAt(i) - 'a'));
        if (Integer.bitCount(bits) > k3003) {
            res = Math.max(res, dfs3003(i + 1, 1 << (s3003.charAt(i) - 'a'), changed) + 1);
        } else {
        ...
        if (changed == 0) {
            for (int b = 0; b < 26; ++b) {
                bits = j | (1 << b);
                if (Integer.bitCount(bits) > k3003) {
                    res = Math.max(res, dfs3003(i + 1, 1 << b, 1) + 1);
                } else {
                    res = Math.max(res, dfs3003(i + 1, bits, 1));
                }
            }
        }
        memo3003.put(key, res);
        return res;
    }
    ...
    public int minimumDeletions(String word, int k) {
        ...
        for (int right = 0; right < 26; ++right) {
            ...
            int cur = 0;
            for (int i = right + 1; i < 26; ++i) {
                cur += Math.min(cnts[i], cnts[right] + k);
            }
            mx = Math.max(mx, s + cur);
        }
