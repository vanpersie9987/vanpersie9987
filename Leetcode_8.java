...
public class Leetcode_8 {
    ...
    // 100029. 和带限制的子多重集合的数目 (Count of Sub-Multisets With Bounded Sum)
    // private int l;
    // private int r;
    // private int[][] memo;
    // private int n;
    ...
    //     }
    //     this.l = l;
    //     this.r = Math.min(r, sum);
    //     this.n = list.size();
    //     this.memo = new int[n][this.r + 1];
    //     for (int i = 0; i < n; ++i) {
    ...
    //     final int MOD = (int) (1e9 + 7);
    //     if (list.get(i)[1] == 0) {
    //         res += ((long) dfs(i + 1, j) * list.get(i)[0]) % MOD;
    //         res %= MOD;
    //     } else {
    //         for (int k = 1; k <= list.get(i)[0]; ++k) {
    //             if (j + list.get(i)[1] * k > r) {
    //                 break;
    //             }
    //             res += dfs(i + 1, j + list.get(i)[1] * k);
    //             res %= MOD;
    //         }
    //     }
    //     return memo[i][j] = res;
