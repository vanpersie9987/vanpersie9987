...
public class Leetcode_11 {
    ...
    public int maxConsistentColumns(int[][] grid, int limit) {
        ...
        for (int i = 0; i < n; ++i) {
            int mx = 0;
            for (int j = i - 1; j >= 0; --j) {
                if (f[j] > mx && check3989(i, j, grid, limit)) {
                    mx = f[j];
                }
            }
            f[i] = mx + 1;
    ...
    private boolean check3989(int i, int j, int[][] grid, int limit) {
        for (int[] r : grid) {
            if (Math.abs(r[i] - r[j]) > limit) {
                return false;
            }
        }
        return true;
    ...
    public double minPrice(int[] prices, int[] discounts) {
        ...
        for (int i = prices.length - 1; i >= 0; --i) {
            if (j >= 0) {
                total += prices[i] * (100D - discounts[j--]) / 100.0D;
            } else {
                total += prices[i];
            }
        }
    ...
    public boolean stoneGameIX(int[] stones) {
        ...
        return check2029(n, cnt.clone()) || check2029(n, new int[] { cnt[0], cnt[2], cnt[1] });
    }

    private boolean check2029(int n, int[] cnt) {
        if (cnt[1] == 0) {
            return false;
        }
        cnt[1]--;
