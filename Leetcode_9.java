...
public class Leetcode_9 {
    ...
    public long minCost(int m, int n, int[][] waitCost) {
        ...
        for (int i = 0; i < m; ++i) {
            Arrays.fill(dis[i], Long.MAX_VALUE >> 1);
        ...
        while (!q.isEmpty()) {
            ...
            if ((int) x == m - 1 && (int) y == n - 1) {
                return d - waitCost[m - 1][n - 1];
            }
            for (int[] dir : dirs) {
                long nx = x + dir[0];
                long ny = y + dir[1];
                if (nx < m && ny < n) {
                    if (d + (nx + 1) * (ny + 1) + waitCost[(int) nx][(int) ny] < dis[(int) nx][(int) ny]) {
                        dis[(int) nx][(int) ny] = d + (nx + 1) * (ny + 1) + waitCost[(int) nx][(int) ny];
                        q.offer(new long[] { d + (nx + 1) * (ny + 1) + waitCost[(int) nx][(int) ny], nx, ny });
                    }
                }
            }
        }
    ...
    public List<String> validateCoupons(String[] code, String[] businessLine, boolean[] isActive) {
        ...
        for (int i = 0; i < code.length; ++i) {
            if (check3606(code[i]) && map.containsKey(businessLine[i]) && isActive[i]) {
                list.add(new String[] { code[i], map.get(businessLine[i]) });
            }
        }
    ...
    private boolean check(int t, int[][] edges, int k, int n) {
        Union3608 union = new Union3608(n);
        for (int[] e : edges) {
            if (e[2] > t) {
                union.union(e[0], e[1]);
            }
        }
        return union.getCnt() >= k;
    }

    public class Union3608 {
        private int[] rank;
        private int[] parent;
        private int cnt;

        public Union3608(int n) {
            this.rank = new int[n];
            this.parent = new int[n];
            Arrays.fill(rank, 1);
            for (int i = 0; i < n; ++i) {
    ...
    class NumArray {
        private SegmentTree t;
        private int n;

        public NumArray(int[] nums) {
            this.t = new SegmentTree(nums);
            this.n = nums.length;
        }
