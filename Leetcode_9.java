...
public class Leetcode_9 {
    ...
    private int cal(int x) {
        this.a = String.valueOf(x).toCharArray();
        this.n = a.length;
        this.memo = new int[n][n][n][k];
        for (int[][][] m : memo) {
            for (int[][] m1 : m) {
                for (int[] m2 : m1) {
                    Arrays.fill(m2, -1);
    ...
    private int dfs(int i, int j, int l, int m, boolean isLimit, boolean isNum) {
        if (i == n) {
            return isNum && j == l && m == 0 ? 1 : 0;
        }
        if (!isLimit && isNum && memo[i][j][l][m] != -1) {
            return memo[i][j][l][m];
        }
        int res = 0;
        if (!isNum) {
            res += dfs(i + 1, j, l, m, false, false);
        }
        int up = isLimit ? a[i] - '0' : 9;
        for (int d = isNum ? 0 : 1; d <= up; ++d) {
            res += dfs(i + 1, j + (((d & 1) == 0) ? 1 : 0), l + (((d & 1) == 1) ? 1 : 0), (m * 10 + d) % k,
                    isLimit && up == d, true);
        }
        if (!isLimit && isNum) {
            memo[i][j][l][m] = res;
