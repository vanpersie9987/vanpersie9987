...
class leetcode_1 :
   ...
   # 107. 二叉树的层序遍历 II (Binary Tree Level Order Traversal II)
    ...
    # 2258. 逃离火灾 (Escape the Spreading Fire)
    def maximumMinutes(self, grid: List[List[int]]) -> int:
        ...
        while left + 1 < right:
            ...
            if check(mid):
                ...
                right = mid
        return left if left < m * n else 10 ** 9         return True
                person.append((x, y))
             for _ in range(size):
                (x, y) = person.popleft()
                for dx, dy in dirs:
                   nx = x + dx
                   ny = y + dy
                   if nx == m - 1 and ny == n - 1 and not vis[nx][ny]:
                      return True
                   if m > nx >= 0 and n > ny >= 0 and not vis[nx][ny] and not fire[nx][ny] and grid[nx][ny] == 0:
                      vis[nx][ny] = True
                      person.append((nx, ny))
             if q:
                size = len(q)
                for _ in range(size):
                   ...
                   for dx, dy in dirs:
                      nx = x + dx
                      ny = y + dy
                      if m > nx >= 0 and n > ny >= 0 and not fire[nx][ny] and grid[nx][ny] == 0:
                         fire[nx][ny] = True
                         q.append((nx, ny))
          return False
       dirs = [[0, 1], [0, -1], [1, 0], [-1, 0]]
       ...
       while person:
          ...
          for _ in range(size):
             ...
             for dx, dy in dirs:
                nx = x + dx
                ny = y + dy
                if not vis[nx][ny] and nx == m - 1 and ny == n - 1:
                   valid = True
                if m > nx >= 0 and n > ny >= 0 and not vis[nx][ny] and not fire[nx][ny] and grid[nx][ny] == 0:
                   vis[nx][ny] = True
                   person.append((nx, ny))
       ...
       while left <= right:
          ...
          if check(mid):
             ...
             right = mid - 1
       return 10 ** 9 if res == m * n else res - 1
