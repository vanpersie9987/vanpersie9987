...
class LcaBinaryLifting:
    ...
    def maxConsistentColumns(self, grid: List[List[int]], limit: int) -> int:
        ...
        for i in range(n):
            ...
            for j in range(i - 1, -1, -1):
                if f[j] > mx and all(abs(r[i] - r[j]) <= limit for r in grid):
                    mx = f[j]
            f[i] = mx + 1
        return max(f)
    ...
    def minCost(self, grid: list[list[int]], k: int) -> int:
        ...
        dis[(0, 0, -1, 4)] = grid[0][0]
        q = []
        # d, x, y, k, last_dir
        q.append((grid[0][0], 0, 0, -1, 4))
        heapq.heapify(q)
        while q:
            d, x, y, cur_k, last_dir = heapq.heappop(q)
            if x == m - 1 and y == n - 1:
                return d
            if d > dis[(d, x, y, cur_k, last_dir)]:
                continue
            for i, (dx, dy) in enumerate(dirs):
                nx, ny = x + dx, y + dy
                if 0 <= nx < m and 0 <= ny < n:
                    nxt_k = cur_k + (last_dir != i)
                    nd = d + grid[nx][ny]
                    if nxt_k <= k and nd < dis[(nx, ny, nxt_k, i)]:
